...
SET spawning_model_pk = NULL
WHERE spawning_model_pk IS NOT NULL
AND watershed_group_code = :'wsg';

UPDATE bcfishpass.streams s
SET rearing_model_pk = NULL
WHERE watershed_group_code = :'wsg'
AND rearing_model_pk IS NOT NULL;
  ...
  SELECT
    ...
    CASE
      WHEN
        ...
        s.access_model_pk IS NOT NULL  -- note: this also ensures only wsg where pk occur are included
      THEN true
      WHEN wsg.model = 'mad' AND
        s.gradient <= pk.spawn_gradient_max AND
        s.mad_m3s > pk.spawn_mad_min AND
        s.mad_m3s <= pk.spawn_mad_max AND
        s.access_model_pk IS NOT NULL
      THEN true
    END AS spawn_pk
...
UPDATE bcfishpass.streams s
SET
  spawning_model_pk = model.spawn_pk
FROM model
...
(
  ...
  WHERE
    s.watershed_group_code = :'wsg' AND          -- on spawning habitat
...
(
  ...
  WHERE spawn.spawning_model_pk IS TRUE
...
(
  ...
  *
  ...
  WHERE spawning_model_pk IS TRUE
