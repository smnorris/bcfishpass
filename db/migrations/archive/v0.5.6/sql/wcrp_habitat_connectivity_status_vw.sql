    ...
    round(
      (
        (
          ...
          coalesce(SUM(ST_Length(s.geom) * .5) FILTER (WHERE h.rearing_sk IS TRUE AND w.sk IS TRUE), 0)
        ) / 1000)::numeric, 2
      ) AS total_km,
    ...
    round(
      (
        (
          ...
          -- add .5 sockeye rearing in lakes (all of it)
          coalesce(SUM(ST_Length(geom) * .5) FILTER (WHERE h.rearing_sk IS TRUE AND w.sk IS TRUE AND barriers_anthropogenic_dnstr IS NULL), 0)
        ) / 1000)::numeric, 2
    ) AS accessible_km
  from bcfishpass.streams s
  inner join bcfishpass.streams_habitat_linear_vw h using (segmented_stream_id)
  ...
  SELECT
    ...
    (
      (
        ...
        coalesce(SUM(ST_Length(s.geom) * .5) FILTER (WHERE h.rearing_co IS TRUE AND w.co IS TRUE AND s.edge_type = 1050), 0) +
        -- add .5 sockeye rearing in lakes (all of it)
        coalesce(SUM(ST_Length(geom) * .5) FILTER (WHERE h.rearing_sk IS TRUE AND w.sk IS TRUE), 0)
      ) / 1000)::numeric, 2
    ) AS total_km,
   ...
   round(
    (
      (
        ...
        -- add .5 sockeye rearing in lakes (all of it)
        coalesce(SUM(ST_Length(geom) * .5) FILTER (WHERE h.rearing_sk IS TRUE AND a.barriers_anthropogenic_dnstr IS NULL), 0)
      ) / 1000)::numeric, 2
    ) AS accessible_km
  from bcfishpass.streams s
  inner join bcfishpass.streams_habitat_linear_vw h using (segmented_stream_id)
...
--select * from bcfishpass.wcrp_habitat_connectivity_status_vw;
