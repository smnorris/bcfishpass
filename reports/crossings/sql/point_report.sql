...
-- ch/co/sk salmon model
  COALESCE(ROUND(((SUM(ST_Length(s.geom)) FILTER (WHERE s.barriers_ch_cm_co_pk_sk_dnstr IS NULL) / 1000)::numeric), 2), 0) AS ch_cm_co_pk_sk_network_km,
  COALESCE(ROUND(((SUM(ST_Length(s.geom)) FILTER (WHERE s.barriers_ch_cm_co_pk_sk_dnstr IS NULL AND (wb.waterbody_type = 'R' OR (wb.waterbody_type IS NULL AND s.edge_type IN (1000,1100,2000,2300))))) / 1000)::numeric, 2), 0) AS ch_cm_co_pk_sk_stream_km,
  COALESCE(ROUND(((SUM(ST_Length(s.geom)) FILTER (
  ...
  COALESCE(ROUND(((SUM(ST_Length(s.geom)) FILTER (WHERE s.barriers_ch_cm_co_pk_sk_dnstr IS NULL AND (s.gradient >= .03 AND s.gradient < .05)) / 1000))::numeric, 2), 0) as ch_cm_co_pk_sk_slopeclass05_km,
  COALESCE(ROUND(((SUM(ST_Length(s.geom)) FILTER (WHERE s.barriers_ch_cm_co_pk_sk_dnstr IS NULL AND (s.gradient >= .05 AND s.gradient < .08)) / 1000))::numeric, 2), 0) as ch_cm_co_pk_sk_slopeclass08_km,
  COALESCE(ROUND(((SUM(ST_Length(s.geom)) FILTER (WHERE s.barriers_ch_cm_co_pk_sk_dnstr IS NULL AND (s.gradient >= .08 AND s.gradient < .15)) / 1000))::numeric, 2), 0) as ch_cm_co_pk_sk_slopeclass15_km,
  COALESCE(ROUND(((SUM(ST_Length(s.geom)) FILTER (WHERE s.barriers_ch_cm_co_pk_sk_dnstr IS NULL AND (s.gradient >= .15 AND s.gradient < .22)) / 1000))::numeric, 2), 0) as ch_cm_co_pk_sk_slopeclass22_km,
  COALESCE(ROUND(((SUM(ST_Length(s.geom)) FILTER (WHERE s.barriers_ch_cm_co_pk_sk_dnstr IS NULL AND (s.gradient >= .22 AND s.gradient < .3)) / 1000))::numeric, 2), 0) as ch_cm_co_pk_sk_slopeclass30_km,
...
(SELECT
  ...
  ROUND(((SUM(COALESCE(uwb.area_lake, 0)) FILTER (WHERE uwb.barriers_ch_cm_co_pk_sk_dnstr IS NULL) +
          SUM(COALESCE(uwb.area_manmade, 0)) FILTER (WHERE uwb.barriers_ch_cm_co_pk_sk_dnstr IS NULL)) / 10000)::numeric, 2) AS ch_cm_co_pk_sk_lakereservoir_ha,
  ROUND(((SUM(COALESCE(uwb.area_wetland, 0)) FILTER (WHERE uwb.barriers_ch_cm_co_pk_sk_dnstr IS NULL)) / 10000)::numeric, 2) AS ch_cm_co_pk_sk_wetland_ha,
