...
UPDATE bcfishpass.streams s
SET model_rearing_pk = NULL
WHERE watershed_group_code = :'wsg'
AND model_rearing_pk IS NOT NULL;
  ...
  SELECT
    ...
    CASE
      WHEN
        ...
        s.model_access_pk IS NOT NULL  -- note: this also ensures only wsg where pk occur are included
      THEN true
      WHEN wsg.model = 'mad' AND
        s.gradient <= pk.spawn_gradient_max AND
        s.mad_m3s > pk.spawn_mad_min AND
        s.mad_m3s <= pk.spawn_mad_max AND
        s.model_access_pk IS NOT NULL
      THEN true
    END AS spawn_pk
...
(
  ...
  WHERE
    ...
    s.model_access_pk IS NOT NULL AND  -- accessibility check
...
(
  ...
  WHERE spawn.model_spawning_pk IS TRUE
...
(
  ...
  *
  ...
  WHERE model_spawning_pk IS TRUE
