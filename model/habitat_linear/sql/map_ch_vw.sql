...
drop materialized view if exists bcfishpass.map_ch_vw;

create materialized view bcfishpass.map_ch_vw as
...
select 
  ...
  case 
    ...
    when 
      ...
      model_rearing_ch is null
    then 'ACCESS' 
    -- potential spawning
    when model_spawning_ch is not null
    then 'SPAWN'
    -- potential rearing (and not spawning)
    when
      model_spawning_ch is null and
      model_rearing_ch is not null
    then 'REAR'
