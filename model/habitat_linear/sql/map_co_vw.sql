...
drop view if exists bcfishpass.map_co_vw;

create view bcfishpass.map_co_vw as
...
select 
  ...
  case 
    ...
    when 
      ...
      model_rearing_co is null
    then 'ACCESS' 
    -- potential spawning
    when model_spawning_co is not null
    then 'SPAWN'
    -- potential rearing (and not spawning)
    when
      model_spawning_co is null and
      model_rearing_co is not null
    then 'REAR'
