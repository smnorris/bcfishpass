...
SELECT
  ...
  round((
    ...
    sum(ST_Length(geom)) FILTER (
     WHERE rearing_model_chinook IS TRUE
     OR rearing_model_steelhead IS TRUE
    ) +
  -- add .5 coho rearing in wetlands
  SUM(ST_Length(geom) * .5) FILTER (
    WHERE rearing_model_coho IS TRUE
    AND edge_type = 1050
    ) +
...
-- total stream length potentially accessible
  ...
  (WHERE accessibility_model_salmon like '%ACCESSIBLE%'
     OR accessibility_model_steelhead like '%ACCESSIBLE%') / 1000)::numeric, 2) as total_accessible_km,
  ...
  -- rearing potentially constrained
  round(

  ((sum(ST_Length(geom)) FILTER (
     WHERE (rearing_model_chinook IS TRUE AND accessibility_model_salmon = 'ACCESSIBLE')
     OR (rearing_model_coho IS TRUE AND accessibility_model_salmon = 'ACCESSIBLE')
     OR (rearing_model_steelhead IS TRUE AND accessibility_model_steelhead = 'ACCESSIBLE')
     OR (rearing_model_sockeye IS TRUE AND accessibility_model_salmon = 'ACCESSIBLE')
    ) +
      -- add .5 coho rearing in wetlands
  SUM(
    ST_Length(geom) * .5)
    FILTER (WHERE rearing_model_coho IS TRUE AND
                  edge_type = 1050 AND
  ...
  SUM(
    ST_Length(geom) * .5)
    FILTER (WHERE spawning_model_sockeye IS TRUE
            AND accessibility_model_salmon = 'ACCESSIBLE')
  ) / 1000)::numeric, 2
