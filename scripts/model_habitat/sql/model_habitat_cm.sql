...
UPDATE bcfishpass.streams s
SET model_rearing_cm = NULL
WHERE watershed_group_code = :'wsg'
AND model_rearing_cm IS NOT NULL;
  ...
  SELECT
    ...
    CASE
      WHEN
        ...
        s.model_access_cm IS NOT NULL  -- note: this also ensures only wsg where cm occur are included
      THEN true
      WHEN wsg.model = 'mad' AND
        s.gradient <= cm.spawn_gradient_max AND
        s.mad_m3s > cm.spawn_mad_min AND
        s.mad_m3s <= cm.spawn_mad_max AND
        s.model_access_cm IS NOT NULL
      THEN true
    END AS spawn_cm
...
(
  ...
  ON h.species_code = 'CM'
  WHERE
    s.watershed_group_code = :'wsg' AND
    s.model_spawning_cm IS TRUE AND          -- on spawning habitat
    s.model_access_cm IS NOT NULL AND  -- accessibility check
...
(
  ...
  WHERE spawn.model_spawning_cm IS TRUE
...
(
  ...
  *
  ...
  WHERE model_spawning_cm IS TRUE
