...
SET spawning_model_cm = NULL
WHERE spawning_model_cm IS NOT NULL
AND watershed_group_code = :'wsg';

UPDATE bcfishpass.streams s
SET rearing_model_cm = NULL
WHERE watershed_group_code = :'wsg'
AND rearing_model_cm IS NOT NULL;
...
UPDATE bcfishpass.streams s
SET
  spawning_model_cm = model.spawn_cm
FROM model
...
(
  ...
  WHERE
    ...
    s.spawning_model_cm IS TRUE AND          -- on spawning habitat
    s.access_model_cm IS NOT NULL AND  -- accessibility check
...
(
  ...
  WHERE spawn.spawning_model_cm IS TRUE
...
(
  ...
  *
  ...
  WHERE spawning_model_cm IS TRUE
