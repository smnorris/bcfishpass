...
WSGS=$($PSQL -AXt -c "SELECT watershed_group_code FROM bcfishpass.param_watersheds where watershed_group_code in ('VICT','COWN','SANJ')")
...
for BARRIERTYPE in ch_co_sk ch_co_sk_b st wct 
do
	parallel --jobs 4 --no-run-if-empty \
		"echo \"SELECT bcfishpass.break_streams(:'point_table', :'wsg');\" | \
...
for BARRIERTYPE in anthropogenic pscis dams dams_hydro ch_co_sk ch_co_sk_b st wct 
do
