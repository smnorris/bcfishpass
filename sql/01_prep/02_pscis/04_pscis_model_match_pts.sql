...
-- NOTE - this scoring method is very crude, use with caution.

DROP TABLE IF EXISTS bcfishpass.pscis_model_match_pts;

CREATE TABLE bcfishpass.pscis_model_match_pts AS
...
SELECT DISTINCT ON (modelled_crossing_id) * FROM
(
...
SELECT DISTINCT ON (stream_crossing_id)
  stream_crossing_id,
  closest_modelled_xing AS modelled_crossing_id,
  dist_m,
  linear_feature_id,
  wscode_ltree,
  localcode_ltree,
...
SELECT
  ...
  m.modelled_crossing_id AS closest_modelled_xing,
  m.dist_m,
  m.linear_feature_id,
  m.wscode_ltree,
  m.localcode_ltree,
  ...
  CASE
    WHEN UPPER(a.stream_name) = UPPER(str.gnis_name) THEN 110
    ELSE 0
  END AS name_score,
...
CROSS JOIN LATERAL
( SELECT modelled_crossing_id, linear_feature_id, wscode_ltree, localcode_ltree, blue_line_key, downstream_route_measure, watershed_group_code, modelled_crossing_type, ST_Distance(b.geom, a.geom) as dist_m
    FROM bcfishpass.modelled_stream_crossings AS b
ORDER BY b.geom <-> a.geom
   LIMIT 5) AS m
INNER JOIN whse_basemapping.fwa_stream_networks_sp str
ON m.linear_feature_id = str.linear_feature_id
WHERE dist_m < 100
...
ORDER BY modelled_crossing_id, total_score DESC NULLS LAST;

-- create idx
ALTER TABLE bcfishpass.pscis_model_match_pts ADD PRIMARY KEY (stream_crossing_id);
CREATE INDEX ON bcfishpass.pscis_model_match_pts (modelled_crossing_id);