...
WHERE
  s.watershed_group_code = :'wsg' AND
  (h.spawning IS TRUE or coalesce(hk.spawning_wct, 0) = 1) AND -- is either modelled or observed spawning
...
(
  SELECT
    ...
    s.gradient,
    case
      when coalesce(h.spawning, false) IS TRUE OR coalesce(hk.spawning_wct, 0) = 1 then true
      else false
    end as spawning,
