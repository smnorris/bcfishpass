...
(SELECT
  ...
  CASE
    WHEN
      ...
      s.model_access_wct IS NOT NULL -- note: this also ensures only wsg where wct occur are included
    THEN true
    WHEN
      wsg.model = 'mad' AND
      s.gradient <= wct.spawn_gradient_max AND
      ...
      s.model_access_wct IS NOT NULL
    THEN true
  END AS spawn_wct
...
(
  ...
  WHERE
    ...
    s.model_access_wct IS NOT NULL AND     -- accessibility check
